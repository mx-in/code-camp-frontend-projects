...
return ''
  ...
  render() {
    ...
    const tweetHref = `https://twitter.com/intent/tweet?hashtags=quotes&related=freecodecamp&text=${encodeURIComponent(
      `"${quote}" ${author}`
    )}`
    return (
      <div id="app">
...
function randomQuote(quoteInfo) {
  return {
    type: RANDOM_QUOTE,
...
function randomQuoteReducer(state = { quote: '', author: '' }, action) {
  switch (action.type) {
    case RANDOM_QUOTE:
      return action.quote
    default:
...
const rootReducer = combineReducers({
  ...
  quoteInfo: randomQuoteReducer
})
const quoteStore = createStore(rootReducer)

function mapQuoteStateToProps(state) {
  return {
...
function mapQuotesDispatchToProps(dispatch) {
  return {
    startLoading: () => {
      dispatch(loading())
    },
